...
function setAndCollectRequestId(config, req, res) {
  let generatedId = req.headers[config.generateAndPropagateHeader];
  if (typeof generatedId === 'undefined') {
    generatedId = randomUUID();
  }
  tracer.currentTrace.context.set(config.generateAndPropagateHeader, generatedId);
  res.setHeader(config.generateAndPropagateHeader, generatedId);
  res.id = generatedId;
  req.id = generatedId;
}
...
function wrappedListener(config, listener) {
  return (req, res, next) => {
    ...
    if (config.setAndPropagateRequestId === true) {
      setAndCollectRequestId(config, req, res);
    }
